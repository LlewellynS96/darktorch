...
class ComplexYOLO(nn.Module):
    ...
    def set_input_dims(self, dims=3):
        ...
        self.layers[0][0] = ComplexConv2d(dims, conv.weight.data.shape[0], kernel_size=3, stride=1, padding=1, bias=False)
        self.to(self.device)
        self.channels = dims
    ...
    def build_modules(self):
        ...
        for block in self.blocks:
            ...
            if block['type'] == 'convolutional':
                ...
                conv_layer = ComplexConv2d(prev_filters, filters, kernel_size, stride, pad, bias=bias)
                # nn.init.normal_(conv_layer.A_weight, 0., 0.001)
                # nn.init.normal_(conv_layer.B_weight, 0., 0.001)
                module.add_module('conv_{0}'.format(index), conv_layer)

                if batch_normalize:
                    bn_layer = ComplexBatchNorm2d(filters)
                    module.add_module('batch_norm_{}'.format(index), bn_layer)

                if activation == 'leaky':
                    activation_layer = ModReLu(filters)
